...
static inline int
table_op_branch_create(Buf* self, Str* user, Str* table, Branch* config)
{
	// [op, user, table, branch]
	auto offset = buf_size(self);
	encode_array(self);
...
static inline uint8_t*
table_op_branch_create_read(uint8_t* op, Str* user, Str* table)
{
	int64_t cmd;
	json_read_array(&op);
	json_read_integer(&op, &cmd);
...
static inline int
table_op_branch_drop(Buf* self, Str* user, Str* table, Str* branch)
{
	// [op, user, table, branch]
	auto offset = buf_size(self);
	encode_array(self);
...
static inline void
table_op_branch_drop_read(uint8_t* op, Str* user, Str* table, Str* branch)
{
	int64_t cmd;
	json_read_array(&op);
	json_read_integer(&op, &cmd);
...
static inline int
table_op_branch_rename(Buf* self, Str* user, Str* name,
                       Str* name_branch,
                       Str* name_branch_new)
{
	// [op, user, name, name_branch, name_branch_new]
	auto offset = buf_size(self);
	encode_array(self);
...
static inline void
table_op_branch_rename_read(uint8_t* op, Str* user, Str* name,
                            Str*     name_branch,
                            Str*     name_branch_new)
{
	int64_t cmd;
