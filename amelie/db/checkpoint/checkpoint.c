...
checkpoint_free(Checkpoint* self)
{
	if (self->workers)
	{
		for (int i = 0; i < self->workers_count; i++)
		{
			...
			notify_close(&worker->on_complete);
		}
...
checkpoint_begin(Checkpoint* self, uint64_t lsn, int workers)
{
	...
	for (int i = 0; i < self->workers_count; i++)
	{
		...
		worker->pid         = -1;
		worker->list_count  = 0;
		notify_init(&worker->on_complete);
		notify_open(&worker->on_complete, &am_self->poller);
...
checkpoint_worker_run(Checkpoint* self, CheckpointWorker* worker)
{
	...
	notify_write(&worker->on_complete);
...
{
	// wait for completion
	poll_read(&self->on_complete.fd, -1);
