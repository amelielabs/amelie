...
emit_recv_upto(Compiler* self, int last, int order)
{
	...
	auto stmt = self->current_scope->stmts.list;
...
hot void
emit_scope(Compiler* self, Scope* scope)
{
	auto current_scope     = self->current_scope;
	auto current_stmt      = self->current;
	auto current_stmt_last = self->last;
	self->current_scope = scope;
	self->current       = NULL;
	self->last          = NULL;

	auto recv_last = -1;
	for (auto stmt = scope->stmts.list; stmt; stmt = stmt->next)
	{
	...
	self->current       = current_stmt;
	self->last          = current_stmt_last;
}
