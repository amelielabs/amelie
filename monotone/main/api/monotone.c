...
mono_free(void* ptr)
{
	...
	case MONO_OBJ_SESSION:
	{
		...
		client_close(&session->client, &main->buf_cache);
		client_free(&session->client, &main->buf_cache);
		break;
	}
...
mono_connect(mono_t* mono, const char* uri)
{
	...
	client_init(client);
	client_set_access(client, ACCESS_CLIENT);
	client_set_mode(client, ACCESS_MODE_ANY);
	int rc;
	if (uri)
	{
		rc = client_set_uri(client, uri);
		if (unlikely(rc == -1))
			goto error;
	}

	// register session
	rc = main_connect(main, client);
	if (unlikely(rc == -1))
		goto error;
	client_set_connected(&session->client, true);
	return session;

error:
	client_free(client, &main->buf_cache);
	free(session);
	return NULL;
}
             ...
             int argc, mono_arg_t* argv)
{
	...
	return client_request(&session->client,
...
{
	auto buf = channel_read(&session->client.src, timeout_ms);
	if (buf == NULL)
	...
	case MSG_CONNECT:
		...
		break;
	case MSG_DISCONNECT:
		client_set_connected(&session->client, false);
