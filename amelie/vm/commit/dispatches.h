...
typedef struct Dispatches Dispatches;
...
dispatches_at(Dispatches* self, int order)
{
...
static inline bool
dispatches_is_first(Dispatches* self)
{
	return self->list_count == 0;
}

static inline void
dispatches_init(Dispatches* self, Gtr* gtr)
{
	self->list_count = 0;
	self->ltrs_count = 0;
	self->gtr        = gtr;
...
static inline void
dispatches_reset(Dispatches* self)
{
	for (auto i = 0; i < self->list_count; i++)
	{
		auto dispatch = dispatches_at(self, i);
...
static inline void
dispatches_free(Dispatches* self)
{
	dispatches_reset(self);
	buf_free(&self->list);
...
dispatches_close(Dispatches* self)
{
...
dispatches_find(Dispatches* self, Part* part)
{
	list_foreach(&self->ltrs)
	{
		auto ltr = list_at(Ltr, link);
...
dispatches_overlaps(Dispatches* self, Dispatches* with)
{
	list_foreach(&self->ltrs)
	{
		auto ltr = list_at(Ltr, link);
		if (dispatches_find(with, ltr->part))
			return true;
	}
	return false;
}
...
hot static inline bool
dispatches_overlaps_dispatch(Dispatches* self, Dispatch* with)
{
	list_foreach(&self->ltrs)
	{
		auto ltr = list_at(Ltr, link);
...
hot static inline void
dispatches_snapshot(Dispatches* self, Access* access)
{
...
hot static inline void
dispatches_send(Dispatches* self, Dispatch* dispatch)
{
	// handle snapshot by starting execution of all prepared ltrs
	// on first send
	if (dispatches_is_first(self) && self->ltrs_count > 0)
	{
		list_foreach(&self->ltrs)
		{
			auto ltr   = list_at(Ltr, link);
	...
	list_foreach(&dispatch->list)
	{
		...
		auto ltr = dispatches_find(self, req->part);
		if (! ltr)
		{
	...
	if (dispatch->close)
		dispatches_close(self);
}

hot static inline Buf*
dispatches_complete(Dispatches* self)
{
	if (! self->ltrs_count)
		return NULL;
