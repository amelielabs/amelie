...
static inline int
table_op_grant(Buf* self, Str* user, Str* name, Str* to, bool grant, uint32_t perms)
{
	// [op, user, name, to, grant, perms]
	auto offset = buf_size(self);
	encode_array(self);
	encode_integer(self, DDL_TABLE_GRANT);
	encode_string(self, user);
	encode_string(self, name);
...
static inline void
table_op_grant_read(uint8_t* op, Str* user, Str* name, Str* to, bool* grant, int64_t* perms)
{
	int64_t cmd;
	json_read_array(&op);
	json_read_integer(&op, &cmd);
	assert(cmd == DDL_TABLE_GRANT);
	json_read_string(&op, user);
	json_read_string(&op, name);
