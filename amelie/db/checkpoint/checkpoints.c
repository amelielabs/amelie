...
void
checkpoints_init(Checkpoints* self, Catalog* catalog)
{
	self->current    = NULL;
	self->list_count = 0;
...
void
checkpoints_free(Checkpoints* self)
{
...
static void
checkpoints_read(Checkpoints* self)
{
	// create directory
	char path[PATH_MAX];
...
void
checkpoints_gc(Checkpoints* self)
{
	for (;;)
	{
...
void
checkpoints_add(Checkpoints* self, uint64_t lsn)
{
...
CheckpointRef*
checkpoints_ref(Checkpoints* self)
{
	spinlock_lock(&self->lock);
...
void
checkpoints_unref(Checkpoints* self, CheckpointRef *ref)
{
	spinlock_lock(&self->lock);
	ref->refs--;
	assert(ref->refs >= 0);
