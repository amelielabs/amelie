...
static void
servers_create_unixsocket(Servers* self)
{
...
static void
servers_create(Servers* self, ServerConfig* config)
{
...
servers_open(Servers* self)
{
	...
	if (pos)
	{
		...
		while (! unpack_array_end(&pos))
		{
			...
			servers_create(self, config);
		}
	}

	// add default unix socket to the repository
	servers_create_unixsocket(self);
}

void
servers_start(Servers* self)
{
