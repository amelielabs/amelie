...
interval_get(Interval* self, char* str, int str_size)
{
	...
	if (self->m != 0)
	{
		...
		if (y != 0)
		{
			...
			size += format(str + size, str_size - size, "{d} {s} ", y, span);
			m = m % 12;
		}
		if (m != 0)
		{
			span = abs(m) != 1? "months": "month";
			size += format(str + size, str_size - size, "{d} {s} ", m, span);
		}
	}
	...
	if (self->d != 0)
	{
		...
		size += format(str + size, str_size - size, "{d} {s} ", d, span);
	}
	...
	if (hours != 0)
	{
		...
		size += format(str + size, str_size - size, "{i64} {s} ", hours, span);
	...
	if (minutes != 0)
	{
		...
		size += format(str + size, str_size - size, "{i64} {s} ", minutes, span);
	...
	if (seconds != 0)
	{
		...
		size += format(str + size, str_size - size, "{i64} {s} ", seconds, span);
		us = us % (1000LL * 1000);
	}
	...
	if (ms != 0)
	{
		size += format(str + size, str_size - size, "{i64} ms ", ms);
		us = us % 1000;
	}

	// microseconds
	if (us != 0)
		size += format(str + size, str_size - size, "{i64} us ", us);
