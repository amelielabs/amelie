...
bool
catalog_deps_validate_compute(Catalog* self, Str* compute, bool error_on_match)
{
	// no recursion
	list_foreach(&self->rels.list)
	{
		...
		case REL_CLONE:
		{
			// clone depends on the compute
			auto clone = clone_of(at);
			dep = str_compare(&clone->table->config->compute, compute);
			break;
		}
		case REL_TABLE:
		{
			// table depends on the compute
			auto table = table_of(at);
			dep = str_compare(&table->config->compute, compute);
			break;
		}
		default:
			break;
		...
		if (dep)
		{
			if (error_on_match)
			{
				error("{s} '{str}.{str}' depends on compute '{str}'",
				      rel_type_of(at->type), at->user, at->name,
				      compute);
			}
			return false;
