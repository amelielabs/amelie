...
typedef struct Backends Backends;
...
static inline void
backends_set_affinity(Backends* self)
{
...
static inline void
backends_start(Backends* self, int count)
{
	...
	if (opt_int_of(&config()->cpu_affinity))
		backends_set_affinity(self);
}

static inline void
backends_stop(Backends* self)
{
...
backends_find(Backends* self, Task* task)
{
	for (int i = 0; i < self->workers_count; i++)
	{
...
static inline void
backends_deploy(Backends* self, Part* part)
{
	auto backend = backends_next(self);
	rpc(&backend->task, MSG_DEPLOY, part);
}

static inline void
backends_deploy_all(Backends* self, Parts* parts)
{
	RpcSet set;
	rpc_set_init(&set);
	defer(rpc_set_free, &set);
	rpc_set_prepare(&set, parts->list_count);
...
static inline void
backends_undeploy(Backends* self, Part* part)
{
...
static inline void
backends_undeploy_all(Backends* self, Parts* parts)
{
	auto count = 0;
	list_foreach(&parts->list)
	{
