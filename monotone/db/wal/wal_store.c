...
hot void
wal_store_write(WalStore* self, Log* log)
{
	...
	// create wal record
	wal_record_create(&self->record, log, lsn);

	// write file and in-memory index
	wal_file_write(self->current, iov_pointer(&self->record.iov),
	               self->record.iov.count);
