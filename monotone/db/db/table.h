...
static inline Part*
table_find_part(Table* self, PartMgr* part_mgr, Uuid* shard)
{
	if (self->config->reference)
		return part_mgr_find_for_table(part_mgr, &self->config->id);

	assert(shard);
	return part_mgr_find_for(part_mgr, shard, &self->config->id);
}
