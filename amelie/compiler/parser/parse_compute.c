...
void
parse_compute_create(Stmt* self)
{
	// CREATE COMPUTE [IF NOT EXISTS] name [DESCRIPTION text]
	auto stmt = ast_compute_create_allocate();
	self->ast = &stmt->ast;
	...
	if (description)
	{
		...
		compute_config_set_description(stmt->config, &text->string);
	}
}

void
parse_compute_drop(Stmt* self)
{
	// DROP COMPUTE [IF EXISTS] name [CASCADE]
	auto stmt = ast_compute_drop_allocate();
	self->ast = &stmt->ast;
...
{
	// ALTER COMPUTE [IF EXISTS] name RENAME TO name
	// ALTER COMPUTE [IF EXISTS] name DESCRIPTION text
	...
	if (stmt_if(self, KRENAME))
	{
		...
		stmt->type = COMPUTE_ALTER_RENAME;
	...
	{
		auto text = stmt_expect(self, KSTRING);
		stmt->type = COMPUTE_ALTER_DESCRIPTION;
