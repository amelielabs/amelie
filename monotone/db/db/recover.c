...
recover_log(Db* self, Transaction* trx, uint64_t lsn, uint8_t** pos)
{
	...
	{
		// find table by uuid
		auto table = table_mgr_find_by_id(&self->table_mgr, &uuid);
		if (unlikely(table == NULL))
			return;

		// replay write
		storage_write(table->storage, trx, type, false, data, data_size);
		return;
	}
