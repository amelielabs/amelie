...
static inline void
coroutines_free(Coroutines* self)
{
...
coroutines_add(Coroutines* self, Coroutine* coro)
{
...
hot static inline void
coroutines_del(Coroutines* self, Coroutine* coro)
{
	assert(self->count > 0);
...
hot static inline void
coroutines_add_ready(Coroutines* self, Coroutine* coro)
{
...
static inline void
coroutines_add_free(Coroutines* self, Coroutine* coro)
{
...
coroutines_find(Coroutines* self, uint64_t id)
{
...
hot static inline void
coroutines_switch(Coroutines* self)
{
...
hot static inline void
coroutines_scheduler(Coroutines* self)
{
...
coroutines_create(Coroutines*  self,
                  ...
                  void*        main_arg)
{
	...
	coroutines_add(self, coro);
	coroutines_add_ready(self, coro);
	return coro;
}
...
coroutine_suspend(Coroutine* coro)
{
	...
	Coroutines* self = coro->coroutines;
	assert(self->current == coro);
	coroutines_switch(self);
}
